...
$stmt = $db->prepare("INSERT IGNORE INTO cbav_hud_snapshots(ts,sym,tf,price,id_tag,features,levels,patterns,ver,symbol)
                      VALUES (?,?,?,?,?,?,?,?,?,?)");
$stmt->bind_param('issdssssss', $ts,$sym,$tf,$pr,$tag,$features,$levels,$patterns,$ver,$sym);
$ok = $stmt->execute();
